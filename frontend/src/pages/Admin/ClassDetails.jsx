...
const ClassDetails = () => {
    ...
    const fetchClassDetails = async () => {
      try {
        ...
        const teachersResponse = await axios.get(
          `http://localhost:3000/api/v1/class/${classId}/teachers`
        );
  ...
  const handleDeleteStudent = async (studentId) => {
    try {
      await axios.delete(
        `http://localhost:3000/api/v1/class/${classId}/students/${studentId}`
      );
  ...
  const handleDeleteSubject = async (subjectId) => {
    try {
      await axios.delete(
        `http://localhost:3000/api/v1/class/${classId}/subjects/${subjectId}`
      );
      setClassDetails((prevDetails) => ({
        ...prevDetails,
        subjects: prevDetails.subjects.filter(
  ...
  const handleDeleteTeacher = async (teacherId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:3000/api/v1/class/${classId}/teachers/${teacherId}`
      );
      setClassDetails((prevDetails) => ({
        ...prevDetails,
        teachers: prevDetails.teachers.filter(
  ...
  const handleAddStudent = async (event) => {
    ...
    try {
      await axios.post(
        `http://localhost:3000/api/v1/class/${classId}/students`,
        {
        ...
        <div className="bg-white rounded-xl shadow-xl p-8">
          ...
          <div className="mb-4">
            <div className="flex border-b">
              <button
                className={`py-2 px-4 ${
                  activeTab === "students" ? "border-b-2 border-indigo-500" : ""
                }`}
              ...
              <button
                className={`py-2 px-4 ${
                  activeTab === "subjects" ? "border-b-2 border-indigo-500" : ""
                }`}
              ...
              <button
                className={`py-2 px-4 ${
                  activeTab === "teachers" ? "border-b-2 border-indigo-500" : ""
                }`}
